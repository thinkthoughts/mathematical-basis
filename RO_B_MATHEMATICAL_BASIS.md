...
sources/persist/      statements/MB_0001_...     tests/MB_0001_...          results/MB_0001_...
...
- A source is preserved as originally written, even where it's wrong. `sources/persist/persist.pdf` still says r(L,Pk) → 24/25. That line is not corrected in place — it's audited, and the audit lives elsewhere.
...
1. `sources/persist/persist.pdf` (v1.1) claimed a specific numerical constant, 24/25, for a specific case (m=6, a=5), based on a numerical table that did not reproduce under its own stated verification code.
