...
\textbf{Draft v3.3} --- extends Draft v3.2 (preserved unchanged as
\texttt{paper-v3.2.pdf}); see the Change Logs at the end of this document
...
\subsection{7. Scope of Application}\label{scope-of-application}

The language of constraints and admissibility used in this paper can
also describe mathematical structure outside the arithmetic setting of
the General Divisor Theorem. Such use should be distinguished from a
direct application of the theorem.

A \emph{general distinction} may identify constraints and an admissible
set without invoking the theorem. A \emph{structural analogy} may
identify similar constraint, admissibility, period, density, or
specification structure between two problems without identifying their
mathematical objects.

Neither, by itself, is a direct application of the General Divisor
Theorem. A direct application requires an explicit mapping to the
theorem's objects \(N,m,a\), from which
\[
d=\mathrm{rad}(\gcd(m,N)), \qquad
R=\frac{\mathrm{rad}(N)}{d}
\]
are defined, verification of the theorem's hypotheses, and conclusions
limited to those supplied by the theorem. In particular, conclusions
involving
\[
\gcd(a,d),\qquad
T_{\min}=\mathrm{lcm}(m,\mathrm{rad}(N)),\qquad
C(N,m)=\frac{d}{\varphi(d)}
\]
require the corresponding arithmetic structure.

Thus the scope distinction is
\[
\boxed{
...
This extends the distinction in Section 4: a shared label or reading
does not by itself specify a structural correspondence, and a structural
analogy does not by itself establish a theorem application.

Further application-scope conventions are recorded in
\texttt{SCOPE.md}.
...
\subsection{\texorpdfstring{Change Log (v3.2 \(\to\) v3.3)}{Change Log (v3.2 -> v3.3)}}\label{change-log-v3.2-v3.3}

Draft v3.3 is a scope-hardening release. The statement and proof of the
General Divisor Theorem, its formulas, specializations, examples, and
numerical results are unchanged from Draft v3.2.
...
\bottomrule\noalign{}
\endlastfoot
Version header & Advanced current draft from v3.2 to v3.3 and identified
\texttt{paper-v3.2.pdf} as the preserved prior draft & VERSIONING \\
New Section 7 & Added application-scope distinction among general
distinction, structural analogy, and direct theorem application &
SCOPE / DOCUMENTATION \\
New Section 7 & Required explicit mapping to \(N,m,a\), definition of
\(d,R\), hypothesis verification, and conclusions limited to the theorem
before claiming direct application & SCOPE / DOCUMENTATION \\
New Section 7 & Connected the application-scope rule to Section 4's
shared-label, shared-reading, and shared-specification distinction and
pointed to \texttt{SCOPE.md} & CROSS-REFERENCE / DOCUMENTATION \\
\end{longtable}

No theorem statement, proof step, formula, specialization, numerical
value, or verification claim was changed in Draft v3.3.
